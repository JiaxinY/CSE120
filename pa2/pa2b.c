 ...
 * changed by calling the kernel function SetSchedPolicy (p), where p is
 * the policy ID (one of the above constants), which is called for the first
 * first time in InitSched (), which currently sets the policy to ARBITRARY
 * (ARBITRARY means exactly what the word "arbitrary" means, i.e., which is
 * that any process may be chosen without any reason or predictability).
 ...
 * mykernel2.c), that you may call from within the above procedures:
 *
