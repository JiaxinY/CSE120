 ...
 * request with n = 0 is typically used to "undo" a process's previous
 * request where some guaranteed rate was previously in place, and after
 * the n = 0 request is made, the guaranteed rate is no longer in effect.
 *
 ...
 * kernel calls your MyRequestCPUrate (p, n) in mykernel2.c, giving you the
 ...
 * of 50% is 5%, and 50%-5% = 45%), 27%, and 9%, respectively.  There no
 * limit as to how much any of the requesting processes can get above
 ...
 * get a chance to run at least once very (k * 100) ticks.
 *
...
void Main ()
{
	if (Fork () == 0) {
		...
		if (Fork () == 0) {
			...
			SlowPrintf (3, "444444444444444444");
			Exit ();
		}

		RequestCPUrate (30);				// process 2
		SlowPrintf (3, "222222222222222222");
		Exit ();
	}
	...
	if (Fork () == 0) {
		...
		SlowPrintf (3, "333333333333333333");
		Exit ();
	}

	RequestCPUrate (40);					// process 1
	SlowPrintf (3, "111111111111111111");
	Exit ();
}
